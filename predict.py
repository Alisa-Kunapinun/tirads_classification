...
import os
import sys
from dataset import create_transform, visualize_augmentations, TiRadsDataset, TiRadsDataset2, TiRadsDataset4GradCam
import numpy as np
...
dir_cam_trans = '/home/alisa/TiradThyroid/Train_Mix/transparent_gb/'
dir_nodule = '/home/alisa/TiradThyroid/Train_Mix/nodule/'
dir_mask_nodule = '/home/alisa/TiradThyroid/Train_Mix/mask/'
dir_results_prob = '/home/alisa/TiradThyroid/Train_Mix/Results/prob/'
...
dir_cam_trans_test = '/home/alisa/TiradThyroid/Test/transparent_gb/'
dir_nodule_test = '/home/alisa/TiradThyroid/Test/nodule/'
dir_mask_nodule_test = '/home/alisa/TiradThyroid/Test/mask/'
dir_results_prob_test = '/home/alisa/TiradThyroid/Test/Results/prob/'
...
model.fc = nn.Linear(2048, num_classes) # assuming that the fc7 layer has 512 neurons, otherwise change it 
...
dataset = TiRadsDataset4GradCam(dir_img, dir_mask, dir_label, transform_test)
dataset_test = TiRadsDataset4GradCam(dir_img_test, dir_mask_test, dir_label_test, transform_test)
train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=True)
val_loader = DataLoader(dataset_test, batch_size=batch_size, shuffle=False, num_workers=1, pin_memory=True, drop_last=True)
...
for epoch in range(1, n_epochs + 1):
    ...
    with tqdm(total=n_train * batch_size, desc=f'Epoch {epoch}/{n_epochs}', unit='img') as pbar:
        for batch in train_loader:
            ...
            pred_composition_prob = sm(pred_composition).to('cpu').detach().numpy() * 100
            pred_echoginicity_prob = sm(pred_echoginicity).to('cpu').detach().numpy() * 100
            pred_shape_prob = sm(pred_shape).to('cpu').detach().numpy() * 100
            pred_margin_prob = sm(pred_margin).to('cpu').detach().numpy() * 100
            pred_macrocal_prob = sm(pred_macrocal).to('cpu').detach().numpy()* 100
            pred_peripheral_prob = sm(pred_peripheral).to('cpu').detach().numpy() * 100
            pred_microcal_prob = sm(pred_microcal).to('cpu').detach().numpy() * 100
            pred_comet_prob = sm(pred_comet).to('cpu').detach().numpy() * 100
            pred_is_malignant_prob = sm(pred_is_malignant).to('cpu').detach().numpy() * 100

            pred_all = np.zeros((24))
            pred_all[0:4] = pred_composition_prob[0]
            pred_all[4:8] = pred_echoginicity_prob[0]
            pred_all[8:10] = pred_shape_prob[0]
            pred_all[10:14] = pred_margin_prob[0]
            pred_all[14:16] = pred_macrocal_prob[0]
            pred_all[16:18] = pred_peripheral_prob[0]
            pred_all[18:20] = pred_microcal_prob[0]
            pred_all[20:22] = pred_comet_prob[0]
            pred_all[22:24] = pred_is_malignant_prob[0]
            pred_all = pred_all / 100.0

            # ex_nodule = dir_nodule + file_name[0]
            # ex_mask = dir_mask_nodule + file_name[0]
            # grad_cam.save_nodule_and_mask(image, ex_nodule, ex_mask)
            xyz = 0
            if xyz != 0:
                for target_idx in range(len(param_idx)):
                    # print(parameters[target_idx])
                    grad_cam.find_target(image, param_idx[target_idx], is_malignant=pred_all[target_idx])
                    ...
                    ex_trans = dir_cam_trans + parameters[target_idx] + '/' + file_name[0]
                    grad_cam.save(ex_cam, ex_gb, ex_cam_gb, ex_trans)

            

            ex_prob = dir_results_prob + file_name[0][:-3] + 'txt'
            with open(ex_prob, 'w') as f:
                f.writelines("composition," + str(pred_composition_prob[0][0]) + ',' + str(pred_composition_prob[0][1]) + ',' +
                        str(pred_composition_prob[0][2]) + ',' + str(pred_composition_prob[0][3]) + '\n')
                f.writelines("echoginicity," + str(pred_echoginicity_prob[0][0]) + ',' + str(pred_echoginicity_prob[0][1]) + ',' +
                        str(pred_echoginicity_prob[0][2]) + ',' + str(pred_echoginicity_prob[0][3]) + '\n')
                f.writelines("shape," + str(pred_shape_prob[0][0]) + ',' + str(pred_shape_prob[0][1]) + '\n')
                f.writelines("margin," + str(pred_margin_prob[0][0]) + ',' + str(pred_margin_prob[0][1]) + ',' +
                        str(pred_margin_prob[0][2]) + ',' + str(pred_margin_prob[0][3]) + '\n')
                f.writelines("macrocalcification," + str(pred_macrocal_prob[0][0]) + ',' + str(pred_macrocal_prob[0][1]) + '\n')
                f.writelines("peripheral," + str(pred_peripheral_prob[0][0]) + ',' + str(pred_peripheral_prob[0][1]) + '\n')
                f.writelines("microcalcification," + str(pred_microcal_prob[0][0]) + ',' + str(pred_microcal_prob[0][1]) + '\n')
                f.writelines("comet," + str(pred_comet_prob[0][0]) + ',' + str(pred_comet_prob[0][1]) + '\n')
                f.writelines("benign/malignant," + str(pred_is_malignant_prob[0][0]) + ',' + str(pred_is_malignant_prob[0][1]) + '\n')
    ...
    with tqdm(total=n_val * batch_size, desc=f'Epoch {epoch}/{n_epochs}', unit='img') as pbar:
        for batch in val_loader:
            ...
            # ex_nodule = dir_nodule_test + file_name[0]
            # ex_mask = dir_mask_nodule_test + file_name[0]
            # grad_cam.save_nodule_and_mask(image, ex_nodule, ex_mask)
            abc = 1
            if abc != 0:
                for target_idx in range(len(param_idx)):
                    # print(file_name[0])
                    # print(parameters[target_idx], pred_all[param_idx[target_idx]])
                    grad_cam.find_target(image, param_idx[target_idx], is_malignant=pred_all[param_idx[target_idx]])
                    ...
                    ex_trans = dir_cam_trans_test + parameters[target_idx] + '/' + file_name[0]
                    grad_cam.save(ex_cam, ex_gb, ex_cam_gb, ex_trans)

            ex_prob = dir_results_prob_test + file_name[0][:-3] + 'txt'
            with open(ex_prob, 'w') as f:
                f.writelines("composition," + str(pred_composition_prob[0][0]) + ',' + str(pred_composition_prob[0][1]) + ',' +
                        str(pred_composition_prob[0][2]) + ',' + str(pred_composition_prob[0][3]) + '\n')
                f.writelines("echoginicity," + str(pred_echoginicity_prob[0][0]) + ',' + str(pred_echoginicity_prob[0][1]) + ',' +
